...
class NewInformationFragment : Fragment() {
    ...
    fun postProFile() {
        ...
        viewmodel.uploaimg(data ,key, object : missionImg {
            override fun success(Url: String) {
                newProfile(Url)
                preferences.updateUrlAvatar(Url)
            }
    ...
    private fun newProfile( url: String) {
        ...
        val country:String= binding.country.text.toString()
        Log.d("kkk","$name")
        var profile = ProFile(key, name,day,month,year,sex,numberPhone,url,country)
        viewmodel.newProfile(profile, object : mission {
            override fun succsess() {
                dialog.dismiss()
                val intent = Intent(requireActivity(), Home::class.java)
                startActivity(intent)
                Log.d("check","${country.length}")
            }
